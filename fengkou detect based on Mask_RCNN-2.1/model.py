...
class ProposalLayer(KE.Layer):
    """根据上一步的anchor scores选择候选框，并进行了非极大值抑制等操作，
    然后根据偏移值对候选框进行了微调，返回筛选后锚框的归一化坐标值。
    ...
    def call(self, inputs):
        # 拿出候选框的前景得分 [Batch, num_rois, 1]
        scores = inputs[0][:, :, 1]
        # 拿出偏移量 [batch, num_rois, 4]
        deltas = inputs[1]
        deltas = deltas * np.reshape(self.config.RPN_BBOX_STD_DEV, [1, 1, 4])
        # Base anchors
        anchors = self.anchors

        # 先根据得分筛选候选框，对剩余的候选框进行微调从而提升表现        
        # 保留多少候选框
        pre_nms_limit = min(6000, self.anchors.shape[0])
        # 按照得分值选出前pre_nms_limit个候选框，拿到索引
...
def detection_targets_graph(proposals, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    # 得到正样本的类别：距离最近的那个标注框的类别就是正样本的类别
    ...
    # 得到正样本的mask:距离最近的mask就是正样本的mask
...
class DetectionTargetLayer(KE.Layer):
    ...
    Inputs:
    proposals: [batch, N, (y1, x1, y2, x2)]归一化之后的rois，可能有不够数而用0填充的rois.
    gt_class_ids: [batch, MAX_GT_INSTANCES] Integer class IDs.真实类别
    gt_boxes: [batch, MAX_GT_INSTANCES, (y1, x1, y2, x2)] in normalized
              coordinates.真实的候选框坐标
    gt_masks: [batch, height, width, MAX_GT_INSTANCES] of boolean type。真实掩码数据
    ...
    def call(self, inputs):
        ...
        # 划分正负样本，标识正样本的类别，计算真实偏移量，生成mask
...
def rpn_graph(feature_map, anchors_per_location, anchor_stride):
    ...
    # 分类计算前景和背景2个得分。anchors_per_location代表的是一个特征点上生成的候选框个数，
...
def fpn_classifier_graph(rois, feature_maps,
    ...
    """
    # ROI Pooling
    # 传入特征图和rois，用rois通过ROIAlign转换为相同大小，并利用特征图计算x
    x = PyramidROIAlign([pool_size, pool_size], image_shape,
                        name="roi_align_classifier")([rois] + feature_maps)
    # 两个1024的全连接层(implemented with Conv2D for consistency)
...
def build_fpn_mask_graph(rois, feature_maps,
    ...
    """
    # ROI Pooling
    # 这里应该是rois经过ROIAlign之后大小的两倍，因为每个特征点上有两个取值（0,1）
    x = PyramidROIAlign([pool_size, pool_size], image_shape,
...
def build_rpn_targets(image_shape, anchors, gt_class_ids, gt_boxes, config):
    ...
    gt_boxes: [num_gt_boxes, (y1, x1, y2, x2)] 人为标注框
...
def data_generator(dataset, config, shuffle=True, augment=True, random_rois=0,
    ...
    while True:
        try:
            ...
            # RPN Targets，rpn_match判断anchors是前景还是背景，rpn_bbox是anchors的4个偏移量
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            # 由于不同阶段特征图尺寸不同，需要将h,w归一化，方便将其映射回原图像
        ...
        # 经过残差网络获取5个阶段的特征图，只使用C2-C5，特征图尺寸不同
        _, C2, C3, C4, C5 = resnet_graph(input_image, config.BACKBONE, stage5=True)
        
        # 自顶向下进行上采样，得到P5-P2       
        # C5经过1*1卷积得到P5
        P5 = KL.Conv2D(256, (1, 1), name='fpn_c5p5')(C5)
        # C5经过1*1卷积，再上采样，然后和C4拼一起得到P4
        P4 = KL.Add(name="fpn_p4add")([
            KL.UpSampling2D(size=(2, 2), name="fpn_p5upsampled")(P5),
            KL.Conv2D(256, (1, 1), name='fpn_c4p4')(C4)])
        # C4经过1*1卷积，再上采样，然后和C3拼一起得到P3
        P3 = KL.Add(name="fpn_p3add")([
            KL.UpSampling2D(size=(2, 2), name="fpn_p4upsampled")(P4),
            KL.Conv2D(256, (1, 1), name='fpn_c3p3')(C3)])
        # C3经过1*1卷积，再上采样，然后和C2拼一起得到P2
        P2 = KL.Add(name="fpn_p2add")([
            KL.UpSampling2D(size=(2, 2), name="fpn_p3upsampled")(P3),
            KL.Conv2D(256, (1, 1), name='fpn_c2p2')(C2)])

        # P2-P5都经过3*3卷积得到P2-P5阶段最终的特征图
        P2 = KL.Conv2D(256, (3, 3), padding="SAME", name="fpn_p2")(P2)
        P3 = KL.Conv2D(256, (3, 3), padding="SAME", name="fpn_p3")(P3)
        P4 = KL.Conv2D(256, (3, 3), padding="SAME", name="fpn_p4")(P4)
        P5 = KL.Conv2D(256, (3, 3), padding="SAME", name="fpn_p5")(P5)
        # P6用于区域框的预生成 is used for the 5th anchor scale in RPN.
        ...
        # 将特征图依次传入rpn层获得特征图上候选框的分类（前景还是背景）+得分+四个偏移量
        layer_outputs = []  # list of lists
        for p in rpn_feature_maps:
            layer_outputs.append(rpn([p]))
        # 每个特征图返回三样东西，e.g. [[a1, b1, c1], [a2, b2, c2]] => [[a1, a2], [b1, b2], [c1, c2]]
        ...
        # 通过建议层从anchors中挑选rois，返回rois归一化的坐标值（若筛选后的rois不满足规定数量会用0填充）
        ...
        if mode == "training":
            ...
            # 通过检测层对roi进行标识，标识roi的类别，roi的偏移，roi的掩码（不够数的会用0填充）
            rois, target_class_ids, target_bbox, target_mask =\
                DetectionTargetLayer(config, name="proposal_targets")([
                    target_rois, input_gt_class_ids, gt_boxes, input_gt_masks])

            # Network Heads
            # 原图上的roi需要映射到特征图上，还原到特征图上得到最终的识别结果
            ...
            # 算anchors的(dy, dx, log(dh), log(dw))损失
            rpn_bbox_loss = KL.Lambda(lambda x: rpn_bbox_loss_graph(config, *x), name="rpn_bbox_loss")(
                [input_rpn_bbox, input_rpn_match, rpn_bbox])
            # 算类别损失
            class_loss = KL.Lambda(lambda x: mrcnn_class_loss_graph(*x), name="mrcnn_class_loss")(
                [target_class_ids, mrcnn_class_logits, active_class_ids])
            # 算rois的(dy, dx, log(dh), log(dw))损失
    ...
    def mold_inputs(self, images):
        ...
        for image in images:
            ...
            # Build image_meta,共10个值，
            # 图片索引号+图片大小+真实图片的左上右下点坐标(原始图片被resise，可能会填充空白)+所属类别onehot
